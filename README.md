...
首页是一个**正规导航站**风格（借鉴「星辰导航」）：玻璃质感顶部导航 + 渐变标题 + 搜索框，
默认**深色主题**，右上角 🌙 可一键切换深/浅色（记忆到浏览器）。

交互采用**标签切换面板**：点导航上的栏目，整块内容区切换为该栏目（带淡入动画），不再下拉、也不再长页滚动；
首屏还有「首页」标签，展示站点标题 + 搜索 + 全部资源的快捷入口。手机端导航可横向滑动，卡片自动变单列，完全自适应。

栏目由 `data/nav.json` 驱动，最多两级：`栏目 → 分组 → 链接`。
带分组的栏目（如「常用工具」下的「系统工具 / 办公软件」）会在该面板内以**分类标签（胶囊）**呈现，点标签可过滤该栏目的卡片。
每个链接可填**网址、图标、说明**，无需上传任何文件，像网址导航站一样使用。
...
字段说明：
- `categories[].groups[].links[]`：带分组的栏目（分组以分区内小标题显示，不在导航下拉）。
