...
首页是一个**正规导航站**风格：玻璃质感顶部导航（彩色胶囊标签）+ 渐变头图（带搜索框）+
各栏目分区。点击导航上的栏目标签，**平滑滚动到下方对应分区并显示卡片**（导航处不再有下拉列表）。
手机端导航标签可横向滑动，卡片自动变单列，完全自适应。

栏目由 `data/nav.json` 驱动，最多两级：`栏目 → 分组 → 链接`。
带分组的栏目（如「常用工具」下的「系统工具 / 办公软件」）会在该分区内以小标题呈现，
不会塞进导航下拉。每个链接可填**网址、图标、说明**，无需上传任何文件，像网址导航站一样使用。
